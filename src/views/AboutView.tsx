        ...
        <div style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 10, padding: 12, marginBottom: 12 }}>
          <div style={{ display: "flex", justifyContent: "space-between", flexWrap: "wrap", gap: 8, alignItems: "center", marginBottom: 8 }}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <button onClick={async () => { const el = document.getElementById("combined-run"); if (el) el.textContent = "Running… (10-20s, 450 samples)"; try { const r = await fetch(`${(import.meta as any).env?.VITE_API_BASE_URL || ""}/skills/managed/metrics/combined`, { headers: { Authorization: `Bearer ${localStorage.getItem("clerk_token") || ""}` }}); const j = await r.json(); if (el) el.textContent = JSON.stringify(j, null, 2); } catch (e) { if (el) el.textContent = String(e); } }} style={{ ...s.btn("secondary"), padding: "6px 10px", fontSize: 11 }}>Run combined (150/sample)</button>
              <button id="export-pdf-btn" onClick={async () => { const btn = document.getElementById("export-pdf-btn") as HTMLButtonElement; if (btn) btn.textContent = "Generating…"; try { const r = await fetch(`${(import.meta as any).env?.VITE_API_BASE_URL || ""}/skills/managed/metrics/export-pdf`, { headers: { Authorization: `Bearer ${localStorage.getItem("clerk_token") || ""}` }}); if (!r.ok) throw new Error(await r.text()); const blob = await r.blob(); const url = URL.createObjectURL(blob); const a = document.createElement("a"); a.href = url; a.download = "guardrail-evidence-pack.pdf"; document.body.appendChild(a); a.click(); a.remove(); URL.revokeObjectURL(url); } catch (e) { alert(String(e)); } finally { if (btn) btn.textContent = "Export PDF"; } }} style={{ ...s.btn("primary"), padding: "6px 12px", fontSize: 11 }}>Export PDF</button>
            </div>
          </div>
