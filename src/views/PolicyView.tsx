...
interface OutputRules { block_toxic_content?: boolean; enforce_schema?: boolean; [key: string]: unknown }
interface ComplianceRules {
  block_medical_advice?: boolean;
  never_discuss_competitors?: boolean;
  full_prompt_logging?: boolean;
  webhook_url?: string;
  blocked_ips?: string[];
  [key: string]: unknown;
}
interface TopicPolicy { blocked_topics?: string[]; [key: string]: unknown }

interface EditablePolicy {
  input_rules?: InputRules | null;
  output_rules?: OutputRules | null;
  compliance_rules?: ComplianceRules | null;
  topic_policy?: TopicPolicy | null;
  llm_backend?: string | null;
  llm_model?: string | null;
}

type RuleSection = "input_rules" | "output_rules" | "compliance_rules";
...
export default function PolicyView({ user }: { user: UserOut }) {
  ...
  useEffect(() => {
    api<EditablePolicy>("/policy").then(setPolicy).catch((e: Error) => setError(e.message));
  }, []);
  ...
  async function save() {
    ...
    try {
      const updated = await api<EditablePolicy>("/policy", { method: "PATCH", body: policy });
      setPolicy(updated);
  ...
  async function reset() {
    ...
    try {
      const updated = await api<EditablePolicy>("/policy/reset", { method: "POST" });
      setPolicy(updated);
  ...
  const toggleRule = (section: RuleSection, key: string) => {
    setPolicy(p => {
      if (!p) return p;
      const sec = (p[section] || {}) as Record<string, unknown>;
      return {
        ...p,
        [section]: { ...sec, [key]: !sec[key] },
      };
    });
  };
  ...
  const toggleTopic = (topic: string) => {
    setPolicy(p => {
      if (!p) return p;
      const blocked = (p.topic_policy?.blocked_topics || []) as string[];
      const next = blocked.includes(topic)
        ? blocked.filter(t => t !== topic)
        : [...blocked, topic];
      return { ...p, topic_policy: { ...(p.topic_policy || {}), blocked_topics: next } };
    });
  };
  ...
  const Rule = ({ label, section, k }: { label: string; section: RuleSection; k: string }) => {
    const sec = (policy[section] || {}) as Record<string, unknown>;
    const on = Boolean(sec[k]);
    return (
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between",
        padding: "12px 0", borderBottom: "1px solid #e7eef6" }}>
        <span style={{ fontSize: 13, color: "#405166", fontWeight: 700 }}>{label}</span>
        <button style={s.toggle(on)}
          ...
          <div style={{ marginTop: 18 }}>
            ...
            <textarea
              ...
              onChange={e => {
                ...
                setPolicy(p => {
                  if (!p) return p;
                  return { ...p, input_rules: { ...p.input_rules, injection_keywords: kws } };
                });
              }}
          ...
          <div style={{ marginTop: 12 }}>
            ...
            <textarea
              ...
              onChange={e => {
                ...
                setPolicy(p => {
                  if (!p) return p;
                  return { ...p, input_rules: { ...p.input_rules, jailbreak_patterns: pats } };
                });
              }}
