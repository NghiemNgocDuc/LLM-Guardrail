...
// Ported from guardrails/skill_messages.py.

const defaultExplanation = "This pattern in agent instructions can expose sensitive data or cause harmful actions when the agent runs tools or sends context to a model."
...
func explainFinding(reasonCode, check string) string {
	if msg, ok := reasonMessages[reasonCode]; ok {
		return msg
	}
	return fmt.Sprintf("%s. %s", check, defaultExplanation)
}
